  ...
  return (
    <div className="media-search__wrapper">
      ...
      <UnstyledList className="media-search__container">
        {hits.map(hit =>
          <li className={`media-search__item`}
              key={hit.objectID}>
...
const MediaHit = ({hit}: { hit: Media }) => {
  ...
  return (
    <MediaWrapper>
      <div className="media-search__card">
        <div
          className={`media-search__card-image${hit.photo ? '' : ' media-search__card-image--no-image'}`}>
          {hit.photo &&
            <img
              src={hit.photo.replace("cardinalsites.acsitefactory.com", "sites.stanford.edu")}
              alt="" loading="lazy"/>
          }
        </div>
  ...
  return (
    <PersonWrapper>
      <div className="media-search__card">
        ...
        <div className="media-search__card-body">
          <div className="media-search__card-title-group">
            <h3 style={{lineHeight: '120%', margin: 0}}>
              <a href={hit.url}>{hit.title}</a>
            </h3>
          </div>
          <div className="media-search__card-short-title">
            {hit.person_short_title}
          </div>
        </div>
      </div>
    </PersonWrapper>
      ...
      <div className="media-search__card">
        ...
        <div className="media-search__card-body">
          ...
          <div className="media-search__card-date">
            {new Date(hit.created * 1000).toLocaleDateString()}
          </div>
        </div>
      </div>
    </NewsWrapper>
